...
#define SRAMCAN_TFQSA                                                                         \
    ((uint32_t)(SRAMCAN_TEFSA + (SRAMCAN_TEF_NBR * SRAMCAN_TEF_SIZE))) /* Tx FIFO/Queue Start \
                                                                          Address */
#define SRAMCAN_SIZE \
    ((uint32_t)(SRAMCAN_TFQSA + (SRAMCAN_TFQ_NBR * SRAMCAN_TFQ_SIZE))) /* Message RAM size */
...
)
{
    ...
    if (hfdcan->State == HAL_FDCAN_STATE_READY) {
        if (RxFifo == FDCAN_RX_FIFO0) {
            ...
            MODIFY_REG(
                hfdcan->Instance->RXGFC, FDCAN_RXGFC_F0OM, (OperationMode << FDCAN_RXGFC_F0OM_Pos)
            );
        } else /* RxFifo == FDCAN_RX_FIFO1 */
        {
            /* Select FIFO 1 Operation Mode */
            MODIFY_REG(
                hfdcan->Instance->RXGFC, FDCAN_RXGFC_F1OM, (OperationMode << FDCAN_RXGFC_F1OM_Pos)
            );
        }
...
)
{
    ...
    if (state == HAL_FDCAN_STATE_BUSY) {
            ...
            if ((hfdcan->Instance->RXF0S & FDCAN_RXF0S_F0FL) == 0U) {
                ...
                if (((hfdcan->Instance->RXF0S & FDCAN_RXF0S_F0F) >> FDCAN_RXF0S_F0F_Pos) == 1U) {
                    if (((hfdcan->Instance->RXGFC & FDCAN_RXGFC_F0OM) >> FDCAN_RXGFC_F0OM_Pos) ==
                        ...
                        /* GetIndex is incremented by one and wraps to 0 in case it overflows the
                         * FIFO size */
        ...
        if (RxLocation == FDCAN_RX_FIFO0) /* Rx element is assigned to the Rx FIFO 0 */
        {
            /* Acknowledge the Rx FIFO 0 that the oldest element is read so that it increments the
             * GetIndex */
            hfdcan->Instance->RXF0A = GetIndex;
        } else /* Rx element is assigned to the Rx FIFO 1 */
        {
            /* Acknowledge the Rx FIFO 1 that the oldest element is read so that it increments the
             * GetIndex */
...
)
{
    ...
    if (state == HAL_FDCAN_STATE_BUSY) {
        ...
        /* Acknowledge the Tx Event FIFO that the oldest element is read so that it increments the
         * GetIndex */
