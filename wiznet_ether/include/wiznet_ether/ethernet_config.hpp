...
wiz_NetInfo netInfo = {
    .mac  = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05}, // MACアドレス
    .ip   = {192, 168, 3, 2}, // IPアドレス
...
uint16_t port_debug     = 5002;
}  // namespace main_board

namespace pc {
uint8_t ip[4]           = {192, 168, 3, 1};  // PCのIPアドレス
uint8_t ip_wifi[4]      = {192, 168, 2, 1};  // PCのIPアドレス
uint16_t port_operation = 5000;              // PCの操作用ポート番号
uint16_t port_feedback  = 5001;              // PCのフィードバック用ポート番号
uint16_t port_debug     = 5002;              // コントローラー用ポート番号 (WiFi)
}  // namespace pc
